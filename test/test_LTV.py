...
T=40
#np.random.seed(1)
S.X0=zonotope(np.array(([0,0,0,0])).reshape(4,1),np.eye(n)*10)
for t in range(T):
    S.A[t]=np.array([[1.3,0.2,-0.1,-0.01],[-0.01,1,-0.01,0],[0.0,0.1,1.2,-0.1],[0,-0.1,-0.1,1]]).reshape(4,4)
    S.B[t]=np.array([[-0.01,0.02,1,0.01]]).reshape(4,1)
